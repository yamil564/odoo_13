...
class AcquirerMercadopago(models.Model):
    ...
    provider = fields.Selection(selection_add=[('mercadopago_pe', 'MercadoPago Perú')])
        ...
        #for (company_id, company_mercadopago_account) in res:
            if company_mercadopago_account:
                ...
                if company_mercadopago_ids:
                    ...
                    mercadopago_view = self.env['ir.model.data'].get_object(cr, uid, 'payment_mercadopago_pe', 'mercadopago_pe_acquirer_button')
                    self.create({
                        'name': 'MercadoPago',
    ...
    def mercadopago_pe_compute_fees(self, amount, currency_id, country_id):
    ...
    def mercadopago_pe_dateformat(self, date):
    ...
    def mercadopago_pe_form_generate_values(self, values):
...
#	            },
	            ...
	            "expires": True,
	            "expiration_date_from": self.mercadopago_pe_dateformat( datetime.datetime.now(tzlocal())-datetime.timedelta(days=1) ),
	            "expiration_date_to": self.mercadopago_pe_dateformat( datetime.datetime.now(tzlocal())+datetime.timedelta(days=31) )
                }
    ...
    def mercadopago_pe_get_reference(self, payment_id=None ):
        ...
        for mp in mps:
            data = mp._mercadopago_pe_get_data(payment_id=payment_id)
...
class TxMercadoPago(models.Model):
    ...
    mercadopago_txn_external_resource_url = fields.Char("external_resource_url")
    mercadopago_txn_verification_code = fields.Char("verification_code")
    mercadopago_txn_payment_method_reference_id = fields.Char("Código de Pago en Agente o Banca por Internet")
    ...
    # --------------------------------------------------
    @api.model
    def _mercadopago_pe_form_get_tx_from_data(self, data, context=None):
...
#charged_back (estado terminal) 	Se realizó un contracargo en la tarjeta de crédito.
    ...
    def _mercadopago_pe_form_validate(self, data):
        #IPN style
        f_data = data
        ...
        _logger.info("_mercadopago_pe_form_validate: external_reference: " +str(external_reference))
