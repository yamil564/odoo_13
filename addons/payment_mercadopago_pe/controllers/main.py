...
#       id = identificador-de-la-operación
        ...
        if (not reference and (topic and str(topic) in ["payment"] and op_id)):
            ...
            reference = request.env["payment.acquirer"].sudo(
            ).mercadopago_pe_get_reference(payment_id=op_id)
        ...
        if reference:
            tx = request.env['payment.transaction'].sudo().search(
                [('reference', '=', reference)])
            _logger.info(
                'mercadopago_pe_validate_data() > payment.transaction founded: %s' % tx.reference)
        ...
        if (tx):
            ...
            ).form_feedback(post, 'mercadopago_pe')

        return res

    @http.route('/payment/mercadopago_pe/hola', type='http', auth='public', methods=['POST', 'GET'], csrf=False)
    def mercadopago_hola(self):
        _logger.info("HOLA MUNDO")

    @http.route('/payment/mercadopago_pe/ipn', type='json', auth='public', methods=['POST', 'GET'], csrf=False)
    ...
    @http.route('/payment/mercadopago_pe/dpn/', type='http', auth="none")
    def mercadopago_pe_dpn(self, **post):
        """ MercadoPago DPN """
        _logger.info('Beginning MercadoPago DPN form_feedback with post data %s',
                     pprint.pformat(post))  # debug
        return_url = self._get_return_url(**post)
        self.mercadopago_pe_validate_data(**post)
        return werkzeug.utils.redirect(return_url)

    @http.route('/payment/mercadopago_pe/cancel', type='http', auth="none")
