...
class StockPicking(models.Model):
    ...
    def action_quotation_send_wp(self):
        ...
        ctx['model_description'] = _('Stock Picking')
            ...
            for rec in self:
                ...
                if rec.partner_id and rec.company_id.inventory_information_in_message:
                    ...
                    if rec.move_ids_without_package:
                        for picking in rec.move_ids_without_package:
                            if picking.quantity_done > 0.00:
                                txt_message += "%0A"+'*'+picking.product_id.display_name+'*'+"%0A"+"*Required Qty:* " + \
                                    str(picking.product_uom_qty)+"%0A"+"*Delivery Qty:* "+str(
                                        picking.quantity_done)+"%0A"+"________________________"+"%0A"
                            else:
                                txt_message += "%0A"+'*'+picking.product_id.display_name+'*'+"%0A" + \
                                    "*Required Qty:* " + \
                    ...
                    if rec.company_id.stock_send_pdf_in_message:
                        ...
                        stock_url = "%0A%0A Click here to download Report : %0A" + \
